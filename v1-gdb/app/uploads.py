...
from .drive_store import upload_to_drive
...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # Save the file (locally) to the upload directory
            ...
            # Upload image to Google Drive (assuming upload_to_drive returns a success flag or file ID)
            #drive_file_id = upload_to_drive(local_file_path, filename)

            # Upload image to Google Drive (Separate directories)
            image_drive_folder = 'ImageUploads'  # Directory for images
            qr_drive_folder = 'QRUploads'  # Directory for QR codes

            # Upload the original image to the 'ImageUploads' folder on Google Drive
            image_drive_file_id = upload_to_drive(local_file_path, filename, image_drive_folder)

            # Upload the QR code to the 'QRUploads' folder on Google Drive
            qr_drive_file_id = upload_to_drive(qr_img_path, f"{file_number}.png", qr_drive_folder)

            # Optionally, clean up the local file after upload to Google Drive
            if image_drive_file_id and qr_drive_file_id:
                os.remove(local_file_path)
                os.remove(qr_img_path)
                # pass

            # flash "QRCode generated successfully!"
